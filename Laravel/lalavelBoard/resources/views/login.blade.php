	...
	<form style="width: 300px;" action="{{ route('login') }}" method="POST">
		...
		{{-- $error->any 에러메시지가 있으면 true, 없으면 false를 반환함 --}}
			...
			<div id="errorMsg" class="form-text text-danger">
				@forelse ($errors->all() as $errorMsg)
					<span>{{ $errorMsg }}</span>
					<br>
				@empty
					<span>{{ session('fromRegister') }}</span>
					<br>
				@endforelse
			</div>
		<div class="mb-3">
		...
		<a href="{{ route('register') }}" class="btn btn-secondary" style="width: 100%;">회원가입</a>
	</form>
	@if (!empty(session('fromRegister')))
